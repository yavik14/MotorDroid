...
public class ControlBT {
	...
	private AcceptThread HiloAceptacion;
	 ...
	 * @param handler
	 *            Un Handler para enviar mensajes de regreso a la actividad
	 *            marcada por el UI
	 * 
	 * */
	public ControlBT(Context context, Handler handler) {
	 ...
	 * 
	 * @param estado
	 *            Un entero definido para cada estado
	 */
	...
	public synchronized void start() {
		...
		if (HiloAceptacion == null) {
			HiloAceptacion = new AcceptThread();
	...
	/**
	 * Para todos los hilos, pone el estado de STATE_NONE donde no esta haciendo
	 * nada
	 */
	...
	private class AcceptThread extends Thread {
		...
		public void run() {
			...
			while (EstadoActual != STATE_CONNECTED) {
				try {
					// Esto es un bloque donde solo obtendremos una conexion o
					// una excepcion
				...
				if (socket != null) {
					synchronized (ControlBT.this) {
						switch (EstadoActual) {
						case STATE_LISTEN:
						case STATE_CONNECTING:
							// Situaction normal. Iniciamos Hilo Conectada
							connected(socket, socket.getRemoteDevice());
							break;
						case STATE_NONE:
						case STATE_CONNECTED:
							// O no esta lista o ya esta conectado. Termina el
							// nuevo socket
							try {
								socket.close();
							} catch (IOException e) {
								Log.e(TAG,
										"No se pudo cerrar el socket no deseado",
										e);
							}
							break;
						}
	...
	/**
	 * Este Hilo correra mientras se intente realizar una conexion de salida con
	 * un dispositivo. Lo hara a traves de la conexion ya sea establecida o
	 * fallada
	 */
	private class ConnectThread extends Thread {
		...
		public void run() {
			...
			try {
				// Aqui solo recibiremos o una conexion establecida o una
				// excepcion
				mmSocket.connect();
			} catch (IOException e) {
				connectionFailed();
				...
				try {
					...
					Log.e(TAG,
							"Imposible cerrar el socket durante el fallo de conexion",
							e2);
				}
				// Inicia el servicio a traves de reiniciar el modo de listening
				ControlBT.this.start();
				return;
			}
	...
	/**
	 * Este hilo corre durante la conexion con un dispositivo remoto. Este
	 * maneja todas las transmisiones de entrada y salida.
	 */
		...
		public void write(byte[] buffer) {
			try {
				OUTPUT_Stream.write(buffer); // Compartir el mensaje enviado con
												// la UI activity
				mHandler.obtainMessage(Motordroid.Mensaje_Escrito, -1, -1,
						buffer).sendToTarget();
			} catch (IOException e) {
		...
		public void run() {
			...
			while (true) { // Mantiene escuchando el InputStream mientras este
				...
				try {
					...
					// byte[] readBufX = (byte[]) buffer; //Construye un String
					// desde los bytes validos en el buffer
					// String readMessageX = new String(readBufX, 0, bytes);//Se
					// envia el readNessagexxx en lugar del buffer pues ya se
					// PARSEO
					mHandler.obtainMessage(Motordroid.Mensaje_Leido, bytes, -1,
